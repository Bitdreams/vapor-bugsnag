...
- `notifier.Close()` **flushes pending reports on shutdown** — the model for draining an async delivery queue before a server task exits. (80%)
...
- **`notifier` object is mandatory-in-practice** and self-identifying: `{ name, version, url }`. Ours: `{ "name": "vapor-bugsnag", "version": "<pkg semver>", "url": "<repo url>" }`. Bugsnag uses this to badge the source SDK.
