...
- Vapor 4 (4.115+), Linux (the consumer runs on AWS Fargate). Also builds on macOS for dev.
...
- Register this middleware **before** (outside) the app's existing `ErrorMiddleware`, so it observes the error but `ErrorMiddleware` still renders the HTTP response. (When integrating into the Tangerine backend specifically, verify the exact registration order in that repo's `configure.swift`.)
...
app.bugsnag.configure(.init(
    ...
    appVersion: TangerineBackendVersion,     // read from the app
    appType: "vapor",
